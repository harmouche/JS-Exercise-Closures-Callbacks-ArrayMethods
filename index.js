...
function processSum(numberList, callback) {
  ...
  return total = total + numberList;}, 0));
}
...
function processContains(item, list, callback) {
  /* CODE HERE */
  let filterArray = list.filter((value) => value ===item);
  let bool = (filterArray[0] === item);
...
function getFullNames(runners) {
  /* CODE HERE */
  let runnerArray = [];
  runners.forEach(runner => runnerArray.push(`${runners.last_name}, ${runners.first_name}`));
  return runnerArray;
}
...
function tallyUpDonations(runners) {
  /* CODE HERE */
  // return runners.reduce((total, runners.donations));

  // let totalDonnationsArray = [0];
  // runners.forEach(runner => totalDonnationsArray.push(runner.donation));
  // let talliedNumber = totalDonnationsArray.reduce((total, num) => total += num);
  // return talliedNumber; 
  let talliedNumber = runners.reduce((total, num) => {
    return total = total + num.donation
  }, 0);
  return talliedNumber;
}
